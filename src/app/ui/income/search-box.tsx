...
const SearchBox = ({
  ...
  const { searchParams, updateQuery } = useIncomeQueryParams();
  ...
  const clearTargets = React.useMemo(
    () => Array.from(new Set([...clearKeys, 'query'])),
    [clearKeys]
  );
  ...
  const clearAll = () => {
    ...
    updateQuery({
      set: selectedYear != null ? { year: selectedYear } : {},
      clear: clearTargets,
      resetPageToOne,
      mode: 'replace',
    });
  ...
  const hasAnythingToClear =
    clearTargets.some((key) => searchParams.has(key)) ||
    (resetPageToOne
  ...
  return (
    ...
    >
      <Input 
        className='min-w-0 flex-2 sm:min-w-[70]'
